...
class Parser(
     ...
     */
    val now : Token
        get() = tokens[index].apply {
            if (debug) {
                "获取当前标记'$text'<$type>".print("语法分析日志[${justNow.last()}]")
            }
        }
    /**
     * 根方法
    ...
    infix fun Token.isOf(type : TokenType) : Boolean {
        return (this.type == type).apply {
            if (debug) {
                "检查'$text'<${this@isOf.type}>的类型为<${type}>,结果为$this".print("语法分析日志[${justNow.last()}]")
            }
        }
    }
    /**
    ...
    infix fun Token.isNotOf(type : TokenType) : Boolean {
        return (this.type != type).apply {
            if (debug) {
                "检查'$text'<${this@isNotOf.type}>的类型不为<${type}>,结果为$this".print("语法分析日志[${justNow.last()}]")
            }
        }
    }
    /**
